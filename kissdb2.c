	...
	unsigned long value_size)
{
	...
	while (fread(httmp,db->hash_table_size_bytes,1,db->f) == 1) {
		...
		if (httmp[db->hash_table_size]) {
			// 通过hash table最后一个元素来获取下一个hash table的开始位置。类似于指针指向。
			// 因为最后一个位置存储的是下一个ht在文件中的偏移量。
			// 这样就可以将所有的ht串起来，通过大小来区分。
...
int KISSDB_get(KISSDB *db,const char *key,void *vbuf)
{
	uint8_t tmp[4096];
	const uint8_t *kptr;
	...
	uint64_t hash = KISSDB_hash(key, klen) % (uint64_t)db->hash_table_size;
	uint64_t offset;
	...
	for(i=0;i<db->num_hash_tables;++i) {
		...
		if (offset) {
			if (fseeko(db->f,offset,SEEK_SET))
				return KISSDB_ERROR_IO;
			// 检查klen是否一致
			n = (long)fread(tmp, LEN_SIZE, 1, db->f); // 读取klen
			if (n <= 0 || (klen != *(unsigned short*)(tmp))) {
				goto get_no_match_next_hash_table;
			}
			//if (n > 0) {
			//	int k = *(unsigned short*)(tmp);
			//	if (klen == k) {
			...
			if (fread(tmp, LEN_SIZE, 1, db->f) != 1) { return KISSDB_ERROR_IO; }
			//if (fread(vbuf,db->value_size,1,db->f) == 1)
			if (fread(vbuf, *(unsigned short*)(tmp), 1, db->f) == 1)
				return 0; /* success */
...
int KISSDB_put(KISSDB *db,const char *key,const void *value, unsigned short vlen)
{
	uint8_t tmp[4096];
	const uint8_t *kptr;
	unsigned long klen,i;
	...
	for(i=0;i<db->num_hash_tables;++i) {
		...
		if (offset) {
			...
			if (fseeko(db->f,offset,SEEK_SET)) // 指针偏移到kv数据处
				return KISSDB_ERROR_IO;
			...
			// 在原来位置写入新值，如果旧址大小小于新值，那么会溢出啊，如何处理呢？todo TODO
			if (fwrite(&vlen, LEN_SIZE, 1, db->f) != 1) { return KISSDB_ERROR_IO; };  // 写入vlen
			...
			// 一次写入 klen，key，vlen， value，这样就可以不需要固定kv的大小了.
			// 描述这些长度，需要2个字节就可以了。
			// 1, write klen
			if (fwrite(&klen, LEN_SIZE, 1, db->f) != 1) { return KISSDB_ERROR_IO; }
			if (fwrite(key, klen, 1, db->f) != 1) { return KISSDB_ERROR_IO; }
			if (fwrite(&vlen, LEN_SIZE, 1, db->f) != 1) { return KISSDB_ERROR_IO; }
			if (fwrite(value, vlen, 1, db->f) != 1) { return KISSDB_ERROR_IO; }
			/*
...
put_no_match_next_hash_table:
	...
	if (fwrite(cur_hash_table,db->hash_table_size_bytes,1,db->f) != 1)
		return KISSDB_ERROR_IO;
	// 新建一块ht后，写入数据
	//char c[15] = {0}; // 写入len时，是1，而不是31，需要注意？？
	if (fwrite(&klen, LEN_SIZE, 1, db->f) != 1) { return KISSDB_ERROR_IO; }
	if (fwrite(key, klen, 1, db->f) != 1) { return KISSDB_ERROR_IO; }
	//memset(c, 0, 15);
	if (fwrite(&vlen, LEN_SIZE, 1, db->f) != 1) { return KISSDB_ERROR_IO; }
